...
int main() {
  ...
  // init
  {
    ...
    forr(i, 0, n) {
      ...
      if (mst.empty()) {
        ans = 1;
        mst.insert(mpr(get<1>(vtup[i]), ans));
        vpr.eb(mpr(get<2>(vtup[i]), ans));
        maxi = max(maxi, ans);
      } else if (x <= (*mst.begin()).first) {
        ans++;
        mst.insert(mpr(get<1>(vtup[i]), ans));
        vpr.eb(mpr(get<2>(vtup[i]), ans));
        maxi = max(maxi, ans);
        ...
        pr = *mst.begin();
        mst.erase(mst.begin());
        mst.insert(mpr(get<1>(vtup[i]), pr.second));
        vpr.eb(mpr(get<2>(vtup[i]), pr.second));
      }
    }
