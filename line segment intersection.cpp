...
bool anyLineTwoSegmentIntersectionCheck(const Point &a, const Point &b,
  ...
  if (a.cross(c, d) == 0 && b.cross(c, d) == 0) {
    ...
    // cout<<getSign(a1)<<pll<<getSign(a2)<<pll<<getSign(a3)<<pll<<getSign(a4)<<ed;

    if (getSign(a1) != getSign(a2) && getSign(a3) != getSign(a4)) {
      return true;
    } else
      return false;
  }
...
int main() {
  fast init {
    ...
    if (anyLineTwoSegmentIntersectionCheck(pa, pb, pc, pd)) {
      cout << "YES" << endl;
    } else
      cout << "NO" << endl;
