...
int main() {
  ...
  // init
  {
    ...
    ll books, maxPrice;
    cin >> books >> maxPrice;
    ll bookPrice[books + 1], bookPage[books + 1];
    vector<vec> dp(books + 1, vec(maxPrice + 1, 0));
    ...
    forr(i, 1, books + 1) { cin >> bookPrice[i]; }

    forr(i, 1, books + 1) { cin >> bookPage[i]; }

    forr(i, 1, books + 1) {
      forr(j, 1, maxPrice + 1) {
        if (j >= bookPrice[i])
          dp[i][j] =
              max(dp[i - 1][j], dp[i - 1][j - bookPrice[i]] + bookPage[i]);
        else
          dp[i][j] = dp[i - 1][j];
      }
    }

    cout << dp[books][maxPrice] << endl;
